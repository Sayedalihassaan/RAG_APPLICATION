...
#     chunks = text+splitter.split_documents(data)
...
#         embeddings = OpenAIEmbeeddings(model = "text-embedding-3-small" , dimensions=1536)
...
def chunk_data(data, chunk_size=512, chunk_overlap=50):
    ...
    # Fixed the typo: text+splitter -> text_splitter
...
def create_embeddings(chunks, persist_directory='./assets/chroma_db'):
    try:
        ...
        # Fixed typo: OpenAIEmbeeddings -> OpenAIEmbeddings
        embeddings = OpenAIEmbeddings(model="text-embedding-3-small", dimensions=1536)
